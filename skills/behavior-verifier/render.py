...
def strip_comments(tpl: str) -> str:
    """Remove every HTML comment except the leading copyright block."""
    return re.sub(r"<!--.*?-->",
                  lambda m: m.group(0) if "Copyright" in m.group(0) else "",
                  tpl, flags=re.DOTALL)
...
def render_html(template: str, data: dict) -> str:
    ...
    # 5. Drop authoring/structural comments (keep the copyright block).
...
def render_txt(data: dict) -> str:
    ...
    out.append(bar)
    out.append("Built on RAISE Framework  |  OWASP Top 10 for LLM Applications 2025  |  "
