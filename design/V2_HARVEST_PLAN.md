...
Re-run the **full 9-target** test suite — *sequentially*, on the merged-schema (Phase-1) skill — and **preserve the run**: commit a `tests/baselines/v0.3-sequential.md` summary (per target: weighted RAISE, the severity counts, and the dominant-pattern one-liner) — optionally the nine findings JSONs too — as the canonical **pre-parallel baseline** that Phase 2's parity gate (§5.2 / §5.3) diffs against. The bar for this gate: every render exits 0, every JSON validates against `schema.py` and `findings.schema.json`, and weighted RAISE scores land in the (already-widened, post-`v0.2.0`-calibration) `tests/README.md` bands with the same dominant themes. Then: CHANGELOG entry, `praxa_version` → `0.3.0`, tag `v0.3.0`, merge the PR.

> **Why a fresh full-9 baseline here, not the existing `/tmp` runs:** the suite runs from this session were on the `v0.2.0` skill + `schema 1.0`, and four of the nine (LangChain SQL, Devika, Sweep, OpenHands) were never re-run after the post-`v0.2.0` recalibration, so they reflect the *intermediate over-conservative* skill — not what's on `main`, and not the Phase-1 skill. Only the FinBot / HelperBot artifacts are durable (they're in `examples/`); the rest live in `/tmp`. Phase 1's gate run is the first clean, complete, durable sequential baseline on the skill the parallel path will actually be built against — so it's the one Phase 2 compares to.
...
7. **CI Python matrix — RESOLVED (folds into #2):** `3.9 / 3.12 / 3.13`.
