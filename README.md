...
| **Running deployment** | Live memory files (`MEMORY.md`, `SOUL.md`, daily logs), session-loaded bootstrap files, configuration files from a deployed instance | Treats live state as the agent's autobiography; scans for behavioral drift, half-wired controls, planned-but-not-deployed gaps, and session-loaded files audited as system prompts. |
| **Operational logs** | **Daily action reports**, **session logs** (JSONL or plaintext), **tool-call audit trails**, **escalation logs**, postmortems, incident records | Treats logs as the ground-truth record of what the agent actually did. Detects tool loops, missed escalations, silent self-handling, undeclared channels in use, and patterns of policy violation invisible in code. |
...
**Logs are first-class evidence.** A daily action report listing every tool invocation, a session JSONL of message blocks and tool calls, or even a plaintext audit log of escalation events — these directly evidence what the agent has *done*, which is often impossible to determine from source code alone. A scan against operational logs can surface tool loops (a 12-hour window producing 85 inbox-poll cycles when ~12 are expected), silent self-handling after escalation failure, undeclared channels actively in use, and self-modifying behavior — none of which are detectable from a repo scan.

The methodology adapts. A repo-only scan covers code-level findings — Manage Your Supply Chain, credential exposure, configuration gaps — but can't observe behavior. A logs-only or behavior-only scan covers Zero Trust violations and policy drift the agent demonstrably committed but can't assess code quality. A scan with multiple input shapes gets the most complete picture, and the report's confidence levels are calibrated accordingly. See [`examples/`](examples/) for repo-shape scans and the test plan in `tests/README.md` (in the source repo) for the breadth of targets we've validated.
